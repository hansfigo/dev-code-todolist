<script lang="ts">
	...
	import { activityDetail, activityTitleStore, activityDetailStore } from '$lib/utils/activityDetail.js';
	import api from '$lib/utils/api';
</script>

<dialog data-cy="modal-delete" id="delete_todo_modal" class="modal">
	<div class="modal-box flex flex-col justify-center items-center">
		<img src="delete-warning.png" alt="" />
		<p class="py-4 text-center">
			Apakah anda yakin menghapus activity <br />“{$todoTitleStore}”?
		</p>
		<form method="dialog" class="flex gap-4 jb">
			...
			<button
				...
				class="button-delete"
				on:click={() => {
					// api.remove($todoIdStore);

					activityDetail.delete($todoIdStore.toString(), $activityDetailStore.id);
					isDeleted.set(true);
